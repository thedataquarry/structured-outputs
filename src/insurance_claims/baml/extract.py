"""
Run a BAML pipeline to extract information from unstructured insurance claims
...
def normalize_enums(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {key: normalize_enums(value) for key, value in data.items()}
    if isinstance(data, list):
        return [normalize_enums(item) for item in data]
    return data


async def extract_claim(record: Dict[str, Any]) -> Dict[str, Any]:
    async with api_semaphore:
        await asyncio.sleep(REQUEST_DELAY)
        claim = await b.ExtractInsuranceClaim(record["claim_text"])
        output = normalize_enums(claim.model_dump())
        output["record_id"] = record["record_id"]
        print(f"✓ Extracted insurance claim for record {record['record_id']}")
        return output


async def process_record(record: Dict[str, Any]) -> Dict[str, Any]:
    try:
        return await extract_claim(record)
