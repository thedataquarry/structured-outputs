"""
Run a BAML pipeline to extract financial entities from unstructured text
...
async def extract_entities(record: Dict[str, Any]) -> Dict[str, Any]:
    async with api_semaphore:
        await asyncio.sleep(REQUEST_DELAY)
        entities = await b.ExtractEntities(record["text"])
        output = entities.model_dump()
        output["record_id"] = record["record_id"]
        print(f"✓ Extracted entities for record {record['record_id']}")
        return output


async def process_record(record: Dict[str, Any]) -> Dict[str, Any]:
    try:
        return await extract_entities(record)
...
async def main(fname: str, start: int, end: int, output_path: str) -> None:
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.fname, start=args.start, end=args.end, output_path=args.output))
