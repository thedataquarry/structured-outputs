...
class FieldCounter:
    ...
    def add_comparison(self, field_name: str, is_match: bool, record_id: int | None = None) -> None:
...
def calculate_field_breakdown(result_record: Dict[str, Any], gold_record: Dict[str, Any]) -> Dict[str, bool]:
    """Calculate field-level accuracy breakdown for flat PII records."""
    field_correctness = {}
    for field, gold_value in gold_record.items():
        if field == "record_id":
            continue
        result_value = result_record.get(field)
        field_correctness[field] = gold_value == result_value
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate PII extraction results")
    parser.add_argument(
        "--gold",
        "-g",
        type=str,
